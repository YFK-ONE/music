		...
		onLoad(options){
			...
			list(options.listId).then((res)=>{
				...
				if(res[1].data.code=='200'){
					this.playlist=res[1].data.playlist;	//排行榜上边属性
					this.privileges=res[1].data.privileges;	//歌单
...
<style scoped>
	...
	.list-head{ display: flex; margin:30rpx;}
		.list-head-img{ width:264rpx; height:264rpx; border-radius: 30rpx; margin-right:42rpx; overflow: hidden; position: relative;}
		...
		.list-head-text view:nth-child(1){ font-size:34rpx; color:white;}
		.list-head-text view:nth-child(2){ display: flex; align-items: center; margin:20rpx 0;}
		...
		.list-music-top{width: 58rpx;font-size: 35rpx;line-height: 35rpx;}
		...
		.list-music-song view:nth-child(2){display: flex;font-size: 20rpx;align-items: center;width: 70vw;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;}
