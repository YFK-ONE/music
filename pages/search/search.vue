			...
			<scroll-view scroll-y="true">
				...
				<block v-else-if="searchType==3">
					<view class="search-suggest">
						...
						<view class="search-suggest-item" v-for="(item,index) in searchSuggest" :key="index" @tap="handleToDetail(item.id)">
							<text class="iconfont iconsearch"></text>{{item.album.name}} - {{item.artists[0].name}}
						</view>
					</view>
				</block>
			</scroll-view>
		...
		methods: {
			...
			handleToSuggest(ev){
				...
				searchWord(value).then((res)=>{
					// console.log(res);
					if(res[1].data.code=='200'){
						this.searchSuggest=res[1].data.result.songs;
						this.searchType=3;
					}
				});
			}
...
<style scoped>
	...
	.search-suggest-head{color: #4575a5; margin-bottom: 74rpx;}
